...
(defun android-tool-bar-configs ()
  (when (and (fboundp 'tool-bar-mode)
    ...
    ;; Put your icon files somewhere normal, e.g. ~/.emacs.d/pen/
    (add-to-list 'image-load-path
                 (expand-file-name "pen" user-emacs-directory))

    (android-general-tool-bar #'tool-bar-add-item nil)))

(define-key key-translation-map [tool-bar apple-keyboard-command]
